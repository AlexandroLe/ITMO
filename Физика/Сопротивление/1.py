...
FPS = 60
ROWS = 12          # всего строк
FULL_ROWS = 6     # строк для горизонтальной части Г
FULL_COLS = 10    # колонн в горизонтальной части
LEG_COLS = 6      # колонн в вертикальной части
SOLDIER_RADIUS = 5
MOVE_SPEED = 200
REPULSION_FORCE = 150
...
# Генерируем Г-образную формацию
soldiers = []
center_x, center_y = WIDTH // 2, HEIGHT // 2
spacing = SOLDIER_RADIUS * 4

total = ROWS * FULL_COLS  # для нормировки факторов можно взять полную ширину
count = 0
for i in range(ROWS):
    if i < FULL_ROWS:
        cols = FULL_COLS
        # горизонтальная часть: центрируем
        for j in range(cols):
            offset_x = (j - (cols - 1) / 2) * spacing
            offset_y = (i - (ROWS - 1) / 2) * spacing
            x = center_x + offset_x
            y = center_y + offset_y
        ...
        # вертикальная часть: выравниваем по левой границе горизонтали
        for j in range(LEG_COLS):
            # левая граница горизонтали
            left_border = center_x - (FULL_COLS - 1) / 2 * spacing
            offset_x = j * spacing + left_border - center_x
            offset_y = (i - (ROWS - 1) / 2) * spacing
            x = center_x + offset_x
            y = center_y + offset_y
            factor = 1 - (count / total)
